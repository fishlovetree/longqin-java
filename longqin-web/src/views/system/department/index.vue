<template>
  <lay-container fluid="true" class="department-box">
    <!-- table -->
    <div class="table-box">
      <lay-table
        :height="`570px`"
      ...
      >
        ...
        <template #option="{ row }">
          ...
          <lay-button
            @click="toRemove(row.departmentId)"
            size="xs"
            border="red"
            border-style="dashed"
          >
    ...
    <lay-layer v-model="visible" :title="title" :area="['700px', '320px']">
      <div style="padding: 20px">
        <lay-form :model="model" ref="layFormRef" :rules="rules">
          ...
          <lay-form-item label="部门名称" prop="departmentName" required>
            <lay-input v-model="model.departmentName"></lay-input>
          </lay-form-item>
          <lay-form-item label="上级部门" prop="parentId">
            <lay-tree-select v-model="model.parentId" :data="dataSource"></lay-tree-select>
          </lay-form-item>
          <lay-form-item label="部门简介" prop="description">
...
import { departmentTree, addDepartment, updateDepartment, deleteDepartment } from "@/api/module/department"
...
function toRemove(departmentId : any) {
    ...
    btn: [
      {
        text: '确定',
        callback: (id: any) => {
          deleteDepartment({departmentId: departmentId}).then((res: any) => {
            if (res.code == 1) {
              layer.msg('您已成功删除')
              layer.close(id)
              handleQuery();
  ...
  layFormRef.value.validate((isValidate: any, model: any, errors: any) => {
    if(isValidate){
      if(model.departmentId === 0){
        // 新建
        addDepartment(model).then((res: any) => {
					if (res.code == 1) {
        ...
        // 修改
        updateDepartment(model).then((res: any) => {
					if (res.code == 1) {
						layer.msg('保存成功', { icon: 1, time: 1000 })
            visible.value = false
						handleQuery();
